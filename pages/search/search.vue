<template>
	<view class="container">
		<view class="search-container tn-margin-top">
			<!-- 搜索框 -->
		...
		<view class="search-associative" v-if="associativeShow">
			<uni-list>
				<uni-list-item  v-for="(item,index) in associativeList" :key="item._id" :ellipsis="1" :title="item.desc" @click="associativeClick(item)" show-extra-icon
...
<script>
	...
	const associativeSearchField = 'desc'; // 联想时，搜索框值检索数据库字段名
	const associativeField = '_id,desc'; // 联想列表每一项携带的字段
	...
	export default {
		data() {
			return {
				...
				associativeList: [],
				keyBoardPopup: false,
				hotWorld: '请输入搜索内容', //	搜索热词，如果没有输入即回车，则搜索热词，但是不会加入搜索记录
				focus: true, //	是否自动聚焦
			}
		},
		created() {
		...
		methods: {
			...
			associativeClick(item) {
				...
				uni.navigateTo({
					url: '/pages/list/detail?_id='+item._id
				})
			},
			loadList(text = '') {
